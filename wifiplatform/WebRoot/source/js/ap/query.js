			...
			.ajax({
				...
				success : function(json) {
					...
					for ( var i = 0; i < json.length; i++) {
						...
						htmlStr += "<td width='7%'><div align='center'>"
								+ ((json[i].province=='北京市')?'':json[i].province)+""
								+ "</div></td>";
						htmlStr += "</tr>";
					}
					htmlStr += "</table>";
			...
			.ajax({
				...
				success : function(json) {
					...
					for ( var i = 0; i < json.length; i++) {
						...
						+ ((json[i].name==null)?'':(json[i].name.length>15?json[i].name.substring(0,15)+"...":json[i].name))
						+ "</div></td>";
						htmlStr += "<td width='7%'><div align='center'><a href='#' onclick='queryClientByAP(\""+json[i].mac+"\");'>浏览</a></div></td>";
						htmlStr += "<td width='7%'><div align='center'>"
							+ json[i].platform
							+ "</div></td>";
						htmlStr += "<td width='7%'><div align='center'>"
...
/** 查看用户 */
function queryClientByAP(mac) {
	window.location="onclien!querybyap.do?mac="+mac;
