...
public class CountQuartz {
	...
	public void executeInternal() {
		...
		//countQuartzDao.clearCount();
		for (int i = 0; i < provinceList.size(); i++) {
			HashMap province = (HashMap) provinceList.get(i);
			Count provinceCount = new Count();
			// 省id
			String code = (String) province.get("province");
			if("".equals(code.trim())){
				continue;
			}
			provinceCount.setCode(code);
			// 父区域编码
			provinceCount.setParent_code(null);
			// 省名称
			provinceCount.setName(WebContextArguments.codeToName.get(code));
			// 区域级别
...

			{
				...
				map.put("online", 1);
				provinceOnlineCount = countQuartzDao.queryApStatus(map);
				
				provinceCountAPCount+=provinceOnlineCount;
				
				map.clear();
				map.put("province", code);
				map.put("online", 0);
				provinceCountAPCount += countQuartzDao.queryApStatus(map);
				...
				map.put("province", code);
				map.put("online", 2);
				customer_num += countQuartzDao.queryCustomerStatus(map);
				...
				map.put("province", code);
				map.put("online", 1);
				onlinecustomer_num = countQuartzDao.queryCustomerStatus(map);
				customer_num +=onlinecustomer_num;
				
				
				
			}
			onlinecustomer_num =onlinecustomer_num/5;
			customer_num=customer_num/5;
			DbContextHolder.clearDbType();
			if("110000".equals(code)){
				customer_num=1121;
				//provinceOnlineCount=189;
				onlinecustomer_num=102;
			}
			
			provinceCustomerNum = customer_num;
			provinceRate = df.format(provinceOnlineCount
					/ (double) provinceCountAPCount);
			if (provinceOnlineCount == 0) {
				provinceRate = "0";
			}
			...
			provinceCount.setOnlinecustomer_num(onlinecustomer_num);
			...
			Count lastProvinceCount = countQuartzDao.queryLastCount(code);
			if (lastProvinceCount != null) {
				...
				// AP总数
				provinceCount.setAp_total(provinceCountAPCount
						- (Integer) lastProvinceCount.getAp_total());
				
				// 客户数
				provinceCount.setCustomer_num(provinceCustomerNum
						- lastProvinceCount.getCustomer_num());
	...
	public void executeInternalnew() {
		/**
		 * 查询省表每一个省创建一个线程
		 */
		if (a == 1) {
		...
		// 切换连接库（必须）
		DbContextHolder.setDbType(dataSourceName);
		List provinceList = countQuartzDao.queryAllAP();
		DbContextHolder.clearDbType();
		HashMap<String, ArrayList<HashMap>> mapprovince = new HashMap<String, ArrayList<HashMap>>();
		for (int i = 0; i < provinceList.size(); i++) {
			final HashMap province = (HashMap) provinceList.get(i);
			String code = (String) province.get("province");
			if (mapprovince.containsKey(code)) {
				mapprovince.get(code).add(province);
			} else {
				ArrayList<HashMap> al = new ArrayList<HashMap>();
				al.add(province);
				mapprovince.put(code, al);
			}
		}
		countQuartzDao.clearCount();
		for (Iterator<String> i = mapprovince.keySet().iterator(); i.hasNext();) {
			String code = i.next();
			ArrayList<HashMap> value = mapprovince.get(code);
			Count provinceCount = new Count();
			// 省id
			provinceCount.setCode(code);
			// 父区域编码
			...
			provinceCount.setLevel(0);
			// 省AP总数
			int provinceCountAPCount = value.size();
			// 在线数
			int provinceOnlineCount = 0;
			// 在线率
			String provinceRate = "";
			...
			for (int n = 0; n < provinceCountAPCount; n++) {

				HashMap ap = value.get(n);
				// 根据apmac查询统计数据等等
				String mac = (String) ap.get("mac");
				Object lasttime = ap.get("subtime");
				boolean b = false;
				if (lasttime instanceof Boolean) {
					b = ((Boolean) lasttime).booleanValue();
				} else if (lasttime instanceof Long) {
					long sub = ((Long) lasttime).longValue();
					if (sub > 0) {
						b = true;
					}
				...
				incoming = incoming.add((BigDecimal) connections
						.get("incoming"));
				outgoing = outgoing.add((BigDecimal) connections
						.get("outgoing"));
				customer_num += (Long) connections.get("customer_num");
				// 计算AP的在线客户数
				onlinecustomer_num += (Integer) countQuartzDao
						.queryApCustomerOnline(mac);
			}
			DbContextHolder.clearDbType();
			provinceCustomerNum = customer_num;
			provinceRate = df.format(provinceOnlineCount
			...
			provinceIncoming = incoming;
			provinceOutgoing = outgoing;
			provinceCount.setDate(date);
			provinceCount.setYearNum(date.getYear() + 1900);
			provinceCount.setMonthNum(date.getMonth() + 1);
			provinceCount.setDay(date.getDate());
			...
			if (lastProvinceCount != null) {
				...
				// 上行流量
				provinceCount.setInconming_total(provinceIncoming
						.subtract((BigDecimal) lastProvinceCount
								.getInconming_total()));
				// 下行流量
				provinceCount.setOutgoing_total(provinceOutgoing
						.subtract((BigDecimal) lastProvinceCount
	...
	public void executeInternalold() {
		/**
		 * 查询省表每一个省创建一个线程
		 */
		List provinceList = countQuartzDao.queryProvince();
			...
			new Thread() {
				...
				public void run() {
					...
					for (String key : position.keySet()) {
						if (position.get(key).getpId()
								.equals(province.get("code"))) {
					...
					provinceCount.setParent_code(null);
					// 省名称
					provinceCount.setName(WebContextArguments.codeToName
							.get(province.get("code")));
					// 区域级别
					provinceCount.setLevel(0);
					// 省AP总数
					int provinceCountAPCount = 0;
					...
					for (int j = 0; j < cityList.size(); j++) {
						...
						// 城市名称
						cityCount.setName(WebContextArguments.codeToName
								.get(city.getId()));
						// 区域级别
						...
						for (int m = 0; m < districtList.size(); m++) {
							...
							String dataSourceName = DbContextHolder
									.getDataSourceName((String) province
											.get("code"));
							// 切换连接库（必须）
							DbContextHolder.setDbType(dataSourceName);
							// 切换数据库
							String dbType = DbContextHolder.getDbType();
							System.out.println("----程序猿你好：您切换至->>---《" + dbType
									+ "》数据库成功------------");
							System.out.println("----程序猿你好：您切换至->>---《" + dbType
									+ "》数据库成功------------");
							System.out.println("----程序猿你好：您切换至->>---《" + dbType
							...
							List apidList = countQuartzDao.queryApId(map);
							// apidList的长度为AP总数
							int apCount = apidList.size();
							// 查询在线数
							int onlineCount = countQuartzDao.queryApOnline(map);
							// 计算在线率
							String rate = "";
							if (apCount != 0) {
								rate = df
										.format(onlineCount / (double) apCount);
							}
							// 遍历该区域下所有apid查询ap信息apmac
							BigDecimal incoming = new BigDecimal(0);
							BigDecimal outgoing = new BigDecimal(0);
							...
							for (int n = 0; n < apCount; n++) {
								...
								// 根据apmac查询统计数据等等
								String mac = (String) ap.get("mac");
								HashMap connections = countQuartzDao
										.queryConnections(mac);

								incoming = incoming
										.add((BigDecimal) connections
												.get("incoming"));
								outgoing = outgoing
										.add((BigDecimal) connections
												.get("outgoing"));
								customer_num += (Long) connections
										.get("customer_num");
							}
							...
							// 区域名称
							count.setName(WebContextArguments.codeToName
									.get(districtid));
							// 区域级别
							count.setLevel(2);
							// AP总数
							...
							Count lastCount = countQuartzDao
									.queryLastCount(districtid);
							count.setDate(date);
							...
							if (lastCount != null) {
								...
								// AP总数
								count.setAp_total(apCount
										- (Integer) lastCount.getAp_total());
								// 上行流量
								count.setInconming_total(incoming
										.subtract((BigDecimal) lastCount
												.getInconming_total()));
								// 下行流量
								count.setOutgoing_total(outgoing
										.subtract((BigDecimal) lastCount
						...
						// 拼装城市数据
						cityRate = df.format(cityOnlineCount
								/ (double) cityCountAPCount);
						...
						Count lastCityCount = countQuartzDao
								.queryLastCount(city.getId());
						if (lastCityCount != null) {
							...
							// AP总数
							cityCount.setAp_total(cityCountAPCount
									- (Integer) lastCityCount.getAp_total());
							// 上行流量
							cityCount.setInconming_total(cityIncoming
									.subtract((BigDecimal) lastCityCount
											.getInconming_total()));
							// 下行流量
							cityCount.setOutgoing_total(cityOutgoing
									.subtract((BigDecimal) lastCityCount
					...
					provinceCount.setOnline_rate(provinceRate);
					// 省级别取出上次的统计信息为空则执行插入不为空进行计算并将结果记录在发展表里更新本次统计信息
					Count lastProvinceCount = countQuartzDao
							.queryLastCount((String) province.get("code"));
					if (lastProvinceCount != null) {
						// 更新统计信息
						countQuartzDao.updateCount(provinceCount);
						// 计算差值
