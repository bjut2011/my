...
public class ApMessageController extends BaseController {
	...
	public ModelAndView queryapMonitor(HttpServletRequest req,
		...
		Account user = (com.sinosoft.account.bean.Account) req.getSession()
				.getAttribute("user");
		Object pcode = user.getPcode();
		if (pcode != null) {
			String dataSourceName = DbContextHolder.getDataSourceName(pcode
	...
	public JSONObject lookApFlow(HttpServletRequest req, HttpServletResponse rep)
		...
		if (queryApFlow == null) {
			...
			long a1 = (Long
					.parseLong((queryApFlow.get("incoming") == null ? "0"
							: queryApFlow.get("incoming").toString())) / 1024);
			long a2 = (Long.parseLong(queryApFlow.get("outgoing") == null ? "0"
					: queryApFlow.get("outgoing").toString())) / 1024;
	...
	private void dispose(List<ApMessage> apMessageList) throws ParseException {
		...
		for (int i = 0; i < apMessageList.size(); i++) {
			...
			if (district.equals("-1")) {
				apMessageList.get(i).setDistrict("");
			}
			Long online = apMessageList.get(i).getOnline();
			if (online != null) {
				if (online == 1) {
					apMessageList.get(i).setIsonLine(true);
				} else if (online == 2) {
					apMessageList.get(i).setIsonLine(false);
				}
			}
			// 处理在线离线显示
			...
			// 处理在线离线显示
			if (apMessageList.get(i).getAuth_plattype() != null) {
				long platform = apMessageList.get(i).getAuth_plattype();
				if (platform == 2) {
					apMessageList.get(i).setPlatform("浙江");
				} else if (platform == 0) {
					apMessageList.get(i).setPlatform("北研");
				} else {
					apMessageList.get(i).setPlatform("ICT");
				}
			} else {
				apMessageList.get(i).setPlatform("ICT");
			}
	...
	public String searchbyid(HttpServletRequest req, HttpServletResponse rep)
		...
		DbContextHolder.clearDbType();
		// 获取切库code
		Object pcodeobj = ((Account) req.getSession().getAttribute("user"))
				.getPcode();
		String pcode = pcodeobj.toString();
	...
	public String highranksearch(HttpServletRequest req, HttpServletResponse rep)
		...
		province_code = (province_code == null || "".equals(province_code)) ? pcode
				: province_code;
	...
	public List initProvince2(HttpServletRequest req, HttpServletResponse rep) {
		rep.setContentType("text/html;charset=utf-8");
		List<AccountDistrictRel> positions = ((Account) req.getSession()
				.getAttribute("user")).getPositions();
		Map map = new HashMap();
		List codeList = new ArrayList();
	...
	public List findDistrictByCode(HttpServletRequest req,
	...
	public List<String> ap_TabShow(HttpServletRequest req,
			HttpServletResponse rep) {
		rep.setContentType("text/html;charset=utf-8");
		...
		String dataSourceName = DbContextHolder.getDataSourceName(acco
				.getPcode());
	...
	public List<String> portal_TF(HttpServletRequest req,
			HttpServletResponse rep) {
		rep.setContentType("text/html;charset=utf-8");
	...
	public List saveSetProtocol(HttpServletRequest req, HttpServletResponse rep) {
		...
		map.put("set_result",
				code + ":" + callback.substring(35, callback.length() - 30)); // 返回结果
	...
	public List saveInternetLimit(HttpServletRequest req,
			HttpServletResponse rep) {
		rep.setContentType("text/html;charset=utf-8");
	...
	public List saveUpgrade(HttpServletRequest req, HttpServletResponse rep) {
		...
		if (cmd_flag == null) {
			...
			if ("startportal".endsWith(cmd_flag)) {
				map.put("cmd_flag", 7);
			} else if ("stopportal".endsWith(cmd_flag)) {
				map.put("cmd_flag", 8);
			} else if ("restartportal".endsWith(cmd_flag)) {
				map.put("cmd_flag", 9);
			} else {
	 ...
	 * */
	@ResponseBody
	@RequestMapping(value = "/apmController!saveSSID")
	public List saveSSID(HttpServletRequest req, HttpServletResponse rep) {
		rep.setContentType("text/html;charset=utf-8");
		Map<String, Object> map = new HashMap<String, Object>();
		String ssid = req.getParameter("ssid");
		ssid.trim();
	 ...
	 * */
	@ResponseBody
	@RequestMapping(value = "/apmController!saveHostname")
	public List saveHostname(HttpServletRequest req, HttpServletResponse rep) {
		rep.setContentType("text/html;charset=utf-8");
		Map<String, Object> map = new HashMap<String, Object>();
	...
	public ModelAndView queryAllApConfig(HttpServletRequest req,
			HttpServletResponse rep, ApMessage apMessage) throws ParseException {
		// 从Session中获取userid
		Account user = (Account) req.getSession().getAttribute("user");
	...
	public String reExecuteProtocal(HttpServletRequest req,
			HttpServletResponse rep) {
		rep.setContentType("text/html;charset=utf-8");
		Map<String, Object> map = new HashMap<String, Object>();
		// 获取页面数据
	...
	public String reExecutePortal(HttpServletRequest req,
			HttpServletResponse rep) {
		rep.setContentType("text/html;charset=utf-8");
		Map<String, Object> map = new HashMap<String, Object>();
		// 获取页面数据
	...
	public String reExecuteInternetLimit(HttpServletRequest req,
			HttpServletResponse rep) {
		rep.setContentType("text/html;charset=utf-8");
		Map<String, Object> map = new HashMap<String, Object>();
		// 名单IP处理
	...
	public String reExecuteInternetLimitMac(HttpServletRequest req,
			HttpServletResponse rep) {
		rep.setContentType("text/html;charset=utf-8");
		Map<String, Object> map = new HashMap<String, Object>();
		// 名单IP处理
	...
	public String reExecuteUpgrade(HttpServletRequest req,
			HttpServletResponse rep) {
		rep.setContentType("text/html;charset=utf-8");
